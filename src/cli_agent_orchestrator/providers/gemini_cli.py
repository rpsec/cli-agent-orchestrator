...
# Regex patterns for Gemini CLI output analysis (module-level constants)
...
# Error indicators
ERROR_INDICATORS = ["Gemini is having trouble responding right now"]
...
class GeminiCliProvider(BaseProvider):
    ...
    def initialize(self) -> bool:
        """Initialize Gemini CLI provider by starting gemini-cli chat command."""
        # Wait for shell to be ready first
        if not wait_for_shell(tmux_client, self.session_name, self.window_name, timeout=10.0):
            raise TimeoutError("Shell initialization timed out after 10 seconds")

        command = f"gemini-cli chat --agent {self._agent_profile}"
    ...
    def extract_last_message_from_script(self, script_output: str) -> str:
        ...
        if not green_arrows:
            raise ValueError("No Gemini CLI response found - no green arrow pattern detected")
